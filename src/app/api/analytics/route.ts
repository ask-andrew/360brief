...
// Shape expected by ModernAnalyticsDashboard
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Pull real data via unified service
    const unified = await fetchUnifiedData(undefined, { startDate: start, endDate: end, useCase: 'analytics' });

    // Aggregate into the AnalyticsData shape
    const emails = unified.emails || [];
    const total_count = emails.length;

    // Simple heuristics: inbound if from != 'me', outbound if from == 'me'
    const inbound_count = emails.filter((e: any) => (e.from || '').toLowerCase() !== 'me').length;
    ...
    const payload: AnalyticsData = {
      ...
      missed_messages: Math.round(total_count * 0.02),
      email_senders,
      time_analytics,
