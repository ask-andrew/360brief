...
export function ExecutiveAnalyticsDashboard() {
  ...
  const [insights, setInsights] = useState<{
    decisionVelocity?: Insight;
    relationshipHealth?: Insight;
    strategicRatio?: Insight;
  }>({});
  ...
  useEffect(() => {
    async function fetchInsights() {
      ...
      try {
        ...
        if (insightsData) {
          const grouped = {
            decisionVelocity: insightsData.find((i: Insight) => i.insight_type === 'decision_velocity'),
            relationshipHealth: insightsData.find((i: Insight) => i.insight_type === 'relationship_health'),
            strategicRatio: insightsData.find((i: Insight) => i.insight_type === 'strategic_vs_reactive'),
          };
          setInsights(grouped);
        }
      } catch (err) {
      ...
      <div className="container mx-auto p-6 max-w-7xl">
        <Card className="border-destructive">
          <CardHeader>
            ...
            <CardDescription>
              {typeof error === 'string' ? error : error?.message || 'Please try refreshing the page or contact support.'}
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
  ...
  const balancedCount = topRelationships.filter((r) => r.balance >= 0.4 && r.balance <= 0.6).length;
    ...
    <div className="container mx-auto p-6 max-w-7xl space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Real-time insights from {(data.total_count as number).toLocaleString()} messages across {(data.processing_metadata as { days_analyzed?: number })?.days_analyzed || 7} days
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <InsightCard
          ...
          value={(data.total_count as number).toLocaleString()}
      ...
      <Card className="border-0 shadow-lg">
        ...
        <CardContent>
          <div className="space-y-3">
            {topRelationships.slice(0, 5).map((rel, index: number) => (
              <RelationshipCard
                key={index}
                email={rel.email}
                balance={rel.balance}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border-0 shadow-lg">
          ...
          <CardContent>
            <div className="space-y-3">
              {((data.channel_analytics as { by_time?: Array<{ hour: string; count: number }> })?.by_time || []).slice(0, 5).map((slot, i: number) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-8 bg-gray-100 rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${(slot.count / Math.max(...((data.channel_analytics as { by_time?: Array<{ hour: string; count: number }> })?.by_time || []).map((t) => t.count))) * 100}%` }}
                    >
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg">
                <div>
                  ...
                  <p className="text-2xl font-bold">{String(data.inbound_count || 0)}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-muted-foreground">Outbound</p>
                  <p className="text-2xl font-bold">{String(data.outbound_count || 0)}</p>
                </div>
              </div>
              <div className="flex justify-between items-center p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg">
                <div>
                  ...
                  <p className="text-2xl font-bold">{String(data.internal_percentage || 0)}%</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-muted-foreground">External</p>
                  <p className="text-2xl font-bold">{String(data.external_percentage || 0)}%</p>
                </div>
              </div>
            </div>
          </CardContent>
